...
router.post("/", rateLimiters.adminWrite, requireAbility(ACTIONS.MANAGE, SUBJECTS.VENUE), validateRequest(schemas.venues.create), venueController.createVenue);
router.get("/", venueController.listVenues);
router.get("/:id", validateRequest(schemas.venues.id), venueController.getVenue);
router.patch("/:id", rateLimiters.adminWrite, requireAbility(ACTIONS.MANAGE, SUBJECTS.VENUE), validateRequest(schemas.venues.update), venueController.updateVenue);
router.delete("/:id", rateLimiters.adminWrite, requireAbility(ACTIONS.MANAGE, SUBJECTS.VENUE), validateRequest(schemas.venues.id), venueController.deleteVenue);
router.get(
  "/:id/schedule",
